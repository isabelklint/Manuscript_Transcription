...
const Field: React.FC<FieldProps> = ({ label, field, metadata, onChange, placeholder = "" }) => (
  <div className="space-y-1">
    ...
    <input
      ...
      className="w-full px-3 py-2 bg-white border border-rose-200 rounded-lg text-xs font-semibold focus:border-rose-400 focus:ring-4 focus:ring-rose-400/5 transition-all"
    />
  </div>
);
...
const PhraseMetadataForm: React.FC<Props> = ({ metadata, onChange }) => {
  return (
    <div className="bg-rose-50/30 p-6 rounded-2xl border border-rose-100 space-y-8 shadow-inner">
      {/* Title Section */}
      ...
      <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100 grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-2">
          <Field label="Library Image Filename (facs)" field="image_source" metadata={metadata} onChange={onChange} placeholder="e.g. cuaderno_1827_p1.jpg" />
          <p className="text-[8px] font-bold text-rose-400 mt-1 uppercase italic">Used for standard TEI @facs attribute and filename generation.</p>
        </div>
      ...
      {/* Export Settings */}
      <div className="bg-rose-50/30 p-4 rounded-xl border border-rose-100 grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-2">
          <Field label="Filename Keyword" field="filename_keyword" metadata={metadata} onChange={onChange} placeholder="e.g. cuaderno" />
          <p className="text-[8px] font-bold text-rose-500 mt-1 uppercase italic">Controls the 'keyword' part of author_keyword_date_page.xml</p>
        </div>
        <div className="md:col-span-2 flex items-center pt-3">
          <div className="bg-rose-100/30 rounded px-2 py-1 border border-rose-200/50">
            <span className="text-[9px] font-black text-rose-600 uppercase">Preview:</span>
            <span className="text-[9px] font-mono text-rose-700 ml-2 italic">..._{metadata.filename_keyword || 'cuaderno'}_...</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-1">
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 bg-white border border-rose-200 rounded-lg text-xs font-semibold focus:border-rose-400 transition-all resize-none"
          />
        </div>
        <div className="space-y-1">
