...
const Field: React.FC<FieldProps> = ({ label, field, metadata, onChange, placeholder = "" }) => (
  <div className="space-y-1">
    ...
    <input
      ...
      className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:border-rose-400 focus:ring-4 focus:ring-rose-400/10 transition-all"
    />
  </div>
);
...
const PhraseMetadataForm: React.FC<Props> = ({ metadata, onChange }) => {
  return (
    <div className="bg-rose-50/20 p-6 rounded-2xl border border-rose-100/60 space-y-8 shadow-inner">
      {/* Title Section */}
      ...
      <div className="bg-rose-50/40 p-4 rounded-xl border border-rose-100 grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-2">
          <Field label="Library Image Filename (facs)" field="image_source" metadata={metadata} onChange={onChange} placeholder="e.g. 000032278_0002.jpg" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-1">
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:border-rose-400 transition-all resize-none"
          />
        </div>
        <div className="space-y-1">
