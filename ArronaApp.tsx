...
const createNewEntry = (lastEntry?: TranscriptionEntry): TranscriptionEntry => {
  ...
  if (lastEntry) {
    ...
    nextLine = parts.length === 2 ? `${parseInt(parts[0]) + 1}.${parts[1]}` : `${parseInt(parts[0]) + 1}`;
  }
  return {
    id: generateId(),
...
const ArronaApp: React.FC = () => {
  ...
  const exportFileName = useMemo(() => {
    ...
    const keyword = sanitize(state.metadata.filename_keyword || 'keyword');
  ...
  const generatedXML = useMemo(() => {
    ...
    const renderEntry = (e: TranscriptionEntry) => {
      ...
      const eid = `p${metadata.pb_n}e${entryIdx.toString().padStart(3, '0')}`;
      ...
      e.kirk_sets.forEach(ks => {
        ...
        ks.daughters.forEach(d => {
          xml += `                <item${d.matches ? ' cert="high"' : ' cert="low"'}>${d.text}</item>\n`;
        });
        xml += `              </list>\n`;
        xml += `            </note>\n`;
      });
...
${renderColumn('col2', '2')}
  ...
  const parseEntryContent = (content: string, layout: 'col1' | 'col2' | 'across'): TranscriptionEntry => {
    ...
    if (variantMatch) variant = { id: generateId(), usg: variantMatch[1], orig: variantMatch[2], norm: variantMatch[3] };
    ...
    while ((kirkMatch = kirkRegex.exec(content)) !== null) {
      ...
      while ((iMatch = itemRegex.exec(kirkContent)) !== null) {
        daughters.push({ id: generateId(), text: iMatch[2], matches: iMatch[1].includes('cert="high"') });
      }
    ...
    return { id: generateId(), layout, line, old_maz, new_maz, ipa, uncertain_maz: unc_maz, old_spa, new_spa, uncertain_spa: unc_spa, eng_gloss, uncertain_eng: unc_eng, variant, kirk_sets, notes };
  };
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        while ((acrossMatch = entryRegex.exec(acrossContent)) !== null) entries.push(parseEntryContent(acrossMatch[1], 'across'));
        ...
        while ((colMatch = colRegex.exec(xml)) !== null) {
          ...
          entryRegex.lastIndex = 0;
          let eMatch;
          while ((eMatch = entryRegex.exec(colMatch[2])) !== null) entries.push(parseEntryContent(eMatch[1], colLayout));
        }
  ...
  return (
    <div className={`flex h-full overflow-hidden bg-slate-100 transition-colors ${isDragging ? 'bg-blue-50' : ''}`}
      ...
      {importSuccess && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-emerald-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white shadow-2xl overflow-hidden">
        <div className="p-4 border-b flex justify-between items-center bg-white z-10 shrink-0">
          <div>
            <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Arrona 1796 · Double-Column Vocabulary</p>
            <p className="text-[9px] font-bold text-slate-400">UVA MSS 01784 · Gates Collection</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => fileInputRef.current?.click()}
              className="px-4 py-1.5 bg-slate-100 text-slate-700 text-[10px] font-black uppercase rounded-lg hover:bg-slate-200 transition-all">Open XML</button>
          </div>
        </div>

        <main className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-10 bg-slate-50/30">
          ...
          <section className="max-w-6xl mx-auto space-y-6 pb-32">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="flex gap-4 text-[10px] font-bold">
                <span className="text-blue-500">Col 1: {state.entries.filter(e => e.layout === 'col1').length}</span>
                <span className="text-purple-500">Col 2: {state.entries.filter(e => e.layout === 'col2').length}</span>
                <span className="text-slate-400">Across: {state.entries.filter(e => e.layout === 'across').length}</span>
              </div>
            </div>
            ...
            <button onClick={addEntry} className="w-full py-16 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-3 text-slate-300 hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50 transition-all group">
              ...
              <span className="text-[11px] font-black uppercase tracking-widest">New Entry</span>
            </button>
          </section>
        </main>
      </div>

      <aside className="w-[480px] bg-[#1a1c23] flex flex-col shrink-0 border-l border-slate-800">
        <div className="p-4 border-b border-white/5 flex justify-between items-center">
          <div className="flex flex-col min-w-0">
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full"></div>TEI P5 · Arrona 1796
            </span>
          ...
          }} className="px-4 py-2 bg-blue-600 text-white text-[10px] font-black uppercase rounded-lg hover:bg-blue-500 shadow-xl shadow-blue-900/40 transition-transform active:scale-95">
            Download XML
          </button>
        </div>
